...
class ScrollAndPinch : MonoBehaviour
{
    ...
    protected Vector3 PlanePosition(Vector2 screenPos)      //터치시
    {
        //position
        var rayNow = Camera.ScreenPointToRay(screenPos);       //현재 클릭한 곳의 레이저를 반환
        if (Plane.Raycast(rayNow, out var enterNow))    //레이저를 씬에 투영하여 대상에 적중되면 true 반환, Raycastfit를 리턴
